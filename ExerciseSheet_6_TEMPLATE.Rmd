...
a)  Briefly describe in one - two sentences what your R package does and
    ...
    The `plotTools` package provides functions for creating custom scatter plots
    and histograms using ggplot2. These functions allow users to specify colors, 
    sizes, and bin numbers, making it easier to generate customized visualizations 
    for data analysis.
...
e.g. `https://github.com/<YOUR_GH_USERNAME>/<PACKAGE_REPOSITORY_NAME>`
    ...
    `https://github.com/Taochuchen/plotTools`
...
```{r} 
remotes::install_github("Taochuchen/plotTools")
```
